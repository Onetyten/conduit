...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fullName = `${profileData?.firstName ?? 'Doe'} ${profileData?.lastName ?? 'John'}`
  const initials = `${profileData?.firstName?.slice(0, 1) ?? ''}${profileData?.lastName?.slice(0, 1) ?? ''}`
          ...
          <div className='w-full flex flex-col md:flex-row gap-2 justify-between'>
            <div className='w-full flex gap-2 items-center '>
              ...
              <div className='flex-1 flex text-sm font-medium flex-col gap-2'>
                <p className='text-2xl sm:text-4xl text-wrap gap break-normal hyphens-auto w-full font-semibold'>{fullName}</p>
